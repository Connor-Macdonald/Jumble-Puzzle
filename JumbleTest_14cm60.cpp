...
void testJumble() {
	...
	// Testing Exeption
	cout << "Tetsing exception on difficulty:\n" << endl;
	try {
		JumblePuzzle test1("CONNOR", "abcd");
	}
	catch (const BadJumbleException& e) {
		cerr << e.what();
		cout << "Diffuculty exception passed\n" << endl;
	}
	cout << "Tetsing exception on word size:\n" << endl;
	try {
		JumblePuzzle test1("a", "easy");
	}
	catch (const BadJumbleException& e) {
		cerr << e.what();
		cout << "Word size exception passed\n" << endl;
	}

	// Testing constructor and size
	JumblePuzzle testE("CONNOR", "easy");
	cout << "Easy size, should be 12: " << testE.getSize() << endl;
	JumblePuzzle testM("CONNOR", "medium");
	cout << "Medium size, Should be 18: " << testM.getSize() << endl;
	JumblePuzzle testH("CONNOR", "hard");
	cout << "Hard size, should be 24: " << testH.getSize() << endl;

	// Testing getters
	showJumble(testM.getJumble(), testM.getSize());
	cout << "Start row of above word: " << testM.getRowPos() << endl;
	cout << "Start column of above word: " << testM.getColPos() << endl; 
	cout << "Direction of above word: " << testM.getDirection() << endl;
